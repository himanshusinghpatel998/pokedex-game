...
const TRAINER_API = "https://considerate-alignment-production-2d97.up.railway.app";
const LEVEL_API = "https://considerate-alignment-production-2d97.up.railway.app";
...
const BattleSim = ({ levelData, levelNumber, isEscapeAllowed = false }) => {
  ...
  const fetchTrainerData = async () => {
    ...
    try {
      const response = await axios.get(
        `${TRAINER_API}/trainer/${trainerId}/data`
      );
  ...
  const initLevelData = () => {
    if (
      levelData?.trainers?.length > 0 &&
      levelData.trainers[0].pokemon?.length > 0
    ) {
  ...
  const executePlayerTurn = async (move) => {
    ...
    const userAttackResult = await calculateDamage(
      userPokemon,
      trainerPokemon,
      move
    );
    ...
    if (
      userAttackResult.effectiveness &&
      userAttackResult.effectiveness !== "normal"
    ) {
  ...
  const executeEnemyTurn = async () => {
    ...
    const randomMove =
      trainerPokemon.moves[
        Math.floor(Math.random() * trainerPokemon.moves.length)
      ];
    ...
    const trainerAttackResult = await calculateDamage(
      trainerPokemon,
      userPokemon,
      randomMove
    );
  ...
  const userSprite = userPokemon?.pokemon_id
    ? `/things/pokemonOpp/${userPokemon.pokemon_id}.png`
    : "/things/pokemonOpp/0.png";
    ...
    <div className="gba-battle-container">
      ...
      <div className="gba-battle-background">
        ...
        <div className="gba-enemy-container">
          <div className="gba-hp-box enemy-hp-box">
            ...
            <div className="gba-health-container">
              <div className="gba-health-bar">
                <motion.div
                  className={`gba-health-fill ${getHealthColorClass(
                    trainerHealthPercent
                  )}`}
                  initial={{ width: "100%" }}
          ...
          <motion.div
            className={`gba-pokemon-sprite enemy-sprite ${
              enemyDamageEffect ? "damage-effect" : ""
            }`}
            animate={
              enemyDamageEffect
        ...
        <div className="gba-player-container">
          <motion.div
            className={`gba-pokemon-sprite player-sprite ${
              playerDamageEffect ? "damage-effect" : ""
            }`}
            animate={
              playerDamageEffect
                ? { x: [-10, 10, -10, 10, 0], opacity: [1, 0.7, 1, 0.7, 1] }
            ...
            <div className="gba-menu-box">
              {showMoveMenu ? (
                  ...
                  {hoveredMove ? (
                    <>
                      ...
                      <p>
                        ...
                        {hoveredMove.type && (
                          <span
                            className={`move-type type-${hoveredMove.type.toLowerCase()}`}
                          >
                            {hoveredMove.type}
                          </span>
                        )}
                      ...
                      <p>
                        PP: {hoveredMove.pp_current}/{hoveredMove.pp_total}
                      </p>
                      <p>
                        Description:{" "}
                ...
                <div className="gba-main-menu">
                  <button onClick={() => handleMainMenuSelection("FIGHT")}>
                    FIGHT
                  </button>
                  <button onClick={() => handleMainMenuSelection("BAG")}>
                    BAG
                  </button>
                  <button onClick={() => handleMainMenuSelection("POKEMON")}>
                    POKéMON
                  </button>
                  {isEscapeAllowed && (
                    <button onClick={() => handleMainMenuSelection("RUN")}>
                      RUN
                    </button>
                  )}
                </div>
